...
PollResult = Tuple[int, str, int, float]
...
INSERT_POLL_CREATE_ID = 'INSERT INTO polls (title, owner_username) VALUES (%s, %s) RETURNING id;'
...
def get_poll_and_vote_results(connection, poll_id: int) -> List[PollResult]:
    with connection:
        with connection.cursor() as cursor:
...
def create_poll(connection, title: str, owner: str, options: List[str]):
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(INSERT_POLL_CREATE_ID, (title, owner))
